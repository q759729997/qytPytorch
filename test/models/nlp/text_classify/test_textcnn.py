"""
    main_module - 文本分类TextCNN，测试时将对应方法的@unittest.skip注释掉.
...
from qytPytorch.models.nlp.text_classify.textcnn import TextCNN  # noqa
...
class TestTextCNN(unittest.TestCase):
    ...
    def test_get_out_shape(self):
        ...
        net = TextCNN(vocab_size=1000, labels_size=2)
        print(net)
    ...
    def test_get_parameter_number(self):
        ...
        net = TextCNN(vocab_size=1000, labels_size=2)
        print(net)
        print(get_parameter_number(net))  # {'total': 5045902, 'trainable': 5045902}
        # vocab_size=1000   时 {'total': 95902, 'trainable': 95902}
        # vocab_size=100000 时 {'total': 5045902, 'trainable': 5045902}
    ...
    def test_train(self):
        ...
        print('vocab len:{}'.format(vocab_size))  # vocab len:624
        test_iter = get_imdb_data_iter(test_data, test_data_vocab, batch_size=8, shuffle=True)
        print('test_iter len:{}'.format(len(test_iter)))  # test_iter len:13
        # 构造模型
        net = TextCNN(vocab_size=vocab_size, labels_size=2)
        print('参数量:{}'.format(get_parameter_number(net)))  # total:77.102 Thousand, trainable:77.102 Thousand
        print(net)
    ...
    def test_train_use_pretrained_embedding(self):
        ...
        # 构造模型
        net = TextCNN(vocab_size=vocab_size, labels_size=2)
        print('参数量:{}'.format(get_parameter_number(net)))  # total:263.152 Thousand, trainable:263.152 Thousand
        # 使用预训练embed初始化
        ...
        embed = load_pretrained_embedding(words=words, pretrained_vocab=glove_embedding)  # There are 1004 oov words.
        net.embedding.weight.data.copy_(embed)
        net.embedding.weight.requires_grad = False  # 直接加载预训练好的, 所以不需要更新它
        print('参数量:{}'.format(get_parameter_number(net)))  # total:263.152 Thousand, trainable:45.902 Thousand
        print(net)
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, net.parameters()))
        loss_func = nn.CrossEntropyLoss()
        # 训练
        ...
        # 保存vocabulary
        save_vocab_words(test_data_vocab, file_name='./data/save/text_classify/textcnn/vocab_words.txt')
        save_serialize_obj(test_data_vocab, filename='./data/save/text_classify/textcnn/vocab.pkl')
    ...
    def test_net_predict(self):
        ...
        # 加载模型
        model_file = './data/save/text_classify/textcnn/model.pkl'
        net = torch.load(model_file)
        print(net)
        # 加载词典
        vocab_file = './data/save/text_classify/textcnn/vocab.pkl'
